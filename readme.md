...
| **多通道告警** | 邮件(SMTP)、钉钉(支持加签)、企业微信、飞书(支持签名)、Telegram、Slack、Discord、通用 Webhook；**告警与恢复均通知**，附触发规则与 MTR 路由快照；每个通道可页面内一键发测试消息 |
...
| **全国延迟地图** | 各省电信/联通/移动三线路延迟着色（可在配置中整体开启/关闭） |
| **检测工具** | 从所有节点向任意域名/IP 同时发起 PING 检测，对比各地连通性 |
...
| 开关全国延迟 | 系统配置 → 基础设置 → 全国延迟功能开关；各省探测 IP 在「全国延迟」页签维护 |
